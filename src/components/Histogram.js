...
class Histogram extends Component {
  ...
  createAttributes() {
    ...
    d3.select(`#chart_${this.props.id}`)
      ...
      .attr(
        "transform",
        `translate(${margin.left}, ${totalH - margin.bottom - padding})`
      )
      .call(xAxis)
    ...
    svg
      ...
      .attr(
        "height",
        d => this.h - padding - this.yAxisScale(this.moviesInBucket(d))
      )
      ...
      .on("mouseover", function (d, i) {
        ...
        tooltip
          .html(
            `${parent.moviesInBucket(d)} movies were rated between ${
              d.x0
            } and ${d.x1}`
          )
  ...
  render() {
    ...
    const maxMoviesInBucket = d3.max(this.ratingsHistogram, d =>
      this.moviesInBucket(d)
    )
